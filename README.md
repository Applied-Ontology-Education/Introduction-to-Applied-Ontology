# Introduction to Applied Ontology  
**Format:** Asynchronous / Online  
**Duration:** 15 Weeks  
**Instructor:** Dr. John Beverley  
**Contact:** ncornetwork@gmail.com

## Course Description
This course introduces participants to the theory and practice of **applied ontology**, a field concerned with the systematic disambiguation of information using machines. Participants learn what ontologies are, how they are built, and how they are used to solve real-world problems. By the end of the course, participants will be able to understand and contribute to ontology development projects, evaluate ontological models, and apply ontological reasoning to interdisciplinary domains such as defense, health, and artificial intelligence.

## Learning Outcomes
By the end of this course, students will be able to:
1. Explain the role of ontologies in enabling **human-human**, **human-machine**, and **machine-machine** interoperability.  
2. Apply methods of **top-down**, **bottom-up**, and **middle-in** modeling to real-world domains.  
3. Understand the philosophical and methodological foundations of **applied ontology**.  
4. Use **hub-and-spoke modularization** to design and integrate interoperable ontology modules.  
5. Analyze concrete ontology applications in **military**, **medical**, and **AI** systems.  
6. Use ontology engineering tools to model and test ontologies.  

## Required Tools
- **Protégé** ([desktop version here](https://protege.stanford.edu/); [browser version here](https://protege.stanford.edu/software.php#web-protege)) 
- **GitHub Account** ([register here](https://github.com/)) 

## Course Structure

### *What Are Ontologies?* (Weeks 1–4)
An exploration of what ontologies are and why they matter. Participants will study the role of ontologies in promoting interoperability and systematic meaning across domains.

| **Week** | **Topic** | **Reading** | **Assignment** |
|:--:|:--|:--|:--|
| 1 | **Human–Human Interoperability** | [Chapter 1, Building Ontologies with BFO](https://direct.mit.edu/books/monograph/4044/Building-Ontologies-with-Basic-Formal-Ontology) | None | 
| 2 | **Human–Machine Interoperability** | [Chapter 2, Semantic Web for the Working Ontologist](https://staff.um.edu.mt/alexiei.dingli/IntroductionToWS/Resources/SWWorkingOntologist.pdf) | [HW #1](https://forms.office.com/r/iRS6rag5Hc) |
| 3 | **Machine–Machine Interoperability** | [Chapter 3, Semantic Web for the Working Ontologist](https://staff.um.edu.mt/alexiei.dingli/IntroductionToWS/Resources/SWWorkingOntologist.pdf) | [HW #2](https://forms.office.com/r/7CL4ZZeqB0) |
| 4 | **Systematic Disambiguation** | None | [HW #3](https://forms.office.com/r/w5ibu04Mc2) |

### *How Are Ontologies Created?* (Weeks 5–9)
Students will learn ontology design principles and modeling methodologies, from abstract frameworks to concrete implementation.

| **Week** | **Topic** | **Reading** | **Assignment** |
|:--:|:--|:--|:--|
| 5 | **Top-Down Modeling** | [Chapter 4, Building Ontologies with BFO](https://direct.mit.edu/books/monograph/4044/Building-Ontologies-with-Basic-Formal-Ontology) | [HW #4](https://forms.office.com/r/F6JRs4h3Le) |
| 6 | **Bottom-Up Modeling** | [Bottom-Up Construction of Ontologies](https://www.researchgate.net/publication/3296767_Bottom-up_construction_of_ontologies) | [HW #5](https://forms.office.com/r/MEU6f89Um4) |
| 7 | **Middle-In Modeling** | [Ontology Development Strategies and the IDO Ecosystem](https://ceur-ws.org/Vol-3603/workshopOIIDDS1.pdf) | [Exam #1](https://forms.office.com/r/vYAhFVpPHY) |
| 8 | **Ontological Commitments** | [Ontological Realism: Coordinated Evolution of Ontologies](https://pmc.ncbi.nlm.nih.gov/articles/PMC3104413/) | [HW #6](https://forms.office.com/r/3JrhGaqaqX) |
| 9 | **Modularization** | [The Common Core Ontologies](https://www.utwente.nl/en/eemcs/fois2024/resources/papers/jensen-et-al-the-common-core-ontologies.pdf) | [HW #7](https://forms.office.com/r/TidEZphYyt) |

### *How Are Ontologies Used?* (Weeks 10–15)
Students will study how ontologies drive interoperability in critical applied domains, integrating prior modeling skills into real-world use cases.

| **Week** | **Topic** | **Reading** | **Assignment** |
|:--:|:--|:--|:--|
| 10 | **Military** | [Joint Doctrine Ontology](https://philpapers.org/archive/MORJDO.pdf) | [HW #8](https://forms.office.com/r/taqvHhJznb) | 
| 11 | **Intelligence** | [Ontology of Information Artifacts in the Intelligence Domain](https://philpapers.org/archive/SMII-16.pdf) | [HW #9](https://forms.office.com/r/GnsGN6nmuZ)  | 
| 12 | **Health** | [The OBO Foundry](https://pmc.ncbi.nlm.nih.gov/articles/PMC2814061/)  | [HW #10](https://forms.office.com/r/j13ebGH1XX) | 
| 13 | **Medicine** | [Promoting Healthy Aging]() | None | 
| 14 | **Artificial Intelligence 1** | [Ontologies in the Era of LLMs](https://www.researchgate.net/publication/376976321_Ontologies_in_the_era_of_large_language_models_-_a_perspective) | None | 
| 15 | **Artificial Intelligence 2** | [Multiple Roles of Ontologies in Explainable AI](https://arxiv.org/pdf/2311.04778) | [Exam #2](https://forms.office.com/r/5VdyTaum0H) | 

## Assessment
Nearly each week, there will be an assignment that must be completed. Assingments will be taken online. The course schedule includes links to each assignment by week it is due. Each assignment is due by Friday evening at 11:59 of the week that it is assigned. 

You must input your full name when you complete the assignment. If you do not, you will not receive credit. 

| **Component** | **Weight** |
|:--|:--:|
| HWs  | 5% each |
| Exams  | 25% each |

## Grading Scale

| **Grade** | **Range** |
|:--:|:--:|
| A | 93–100 |
| A– | 90–92 |
| B+ | 87–89 |
| B | 83–86 |
| B– | 80–82 |
| C+ | 77–79 |
| C | 73–76 |
| D | 65–72 |
| F | <65 |